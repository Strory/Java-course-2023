...
public class OneThreadRenderer implements Renderer {
    ...
    public OneThreadRenderer(FunctionProvider functionProvider) {
        this.functionProvider = functionProvider;
    }

    @Override
    public void render(int numCount, int iterCount, int width, int height, int symmetry, double gammaCorrect) {
        Pixel[][] pixels = generatePixelsArray(numCount, iterCount, width, height, symmetry);
    ...
    @SuppressWarnings("MagicNumber")
    private Pixel[][] generatePixelsArray(int numCount, int iterCount, int width, int height, int symmetry) {
                ...
                if (symmetry == 0) {
                    ...
                    if (pointX < width && pointY < height && pointX > 0 && pointY > 0) {
                        if (pixels[pointY][pointX] == null) {
                            ...
                            int red = (pixels[pointY][pointX].getRed() + affinFunctions[affinIndex].red()) / 2;
                            int green = (pixels[pointY][pointX].getGreen() + affinFunctions[affinIndex].green()) / 2;
                            int blue = (pixels[pointY][pointX].getBlue() + affinFunctions[affinIndex].blue()) / 2;
